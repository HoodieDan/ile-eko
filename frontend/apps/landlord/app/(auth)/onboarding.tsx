import { LinearGradient } from 'expo-linear-gradient';
import { useRouter } from 'expo-router';
import React, { useRef, useState } from 'react';
import {
  Pressable,
  ScrollView,
  View,
  useWindowDimensions,
  type NativeScrollEvent,
  type NativeSyntheticEvent,
} from 'react-native';
import { AILabel, Button, Icon, Logo, Screen, Text, colors, radii, type IconName } from '@ile-eko/ui';

interface Slide {
  icon: IconName;
  title: string;
  body: string;
  ai: boolean;
  art: [string, string];
  tile: string;
}

const SLIDES: Slide[] = [
  {
    icon: 'layers',
    title: 'Everything in one place',
    body: 'Properties, tenants, rent and your on-the-ground team — managed from a single home screen.',
    ai: false,
    art: ['#E6F0EA', '#DBE8E3'],
    tile: colors.primary,
  },
  {
    icon: 'spark',
    title: 'AI that watches your portfolio',
    body: 'Smart rent pricing, default predictions and a daily briefing flag what needs you — before it becomes a problem.',
    ai: true,
    art: ['#ECE9FB', '#DCD6F8'],
    tile: colors.ai,
  },
  {
    icon: 'globe',
    title: 'Built for landlords abroad',
    body: 'Full oversight from anywhere in the world. Delegate to trusted caretakers with permissions you control.',
    ai: false,
    art: ['#E6F0EA', '#DBE8E3'],
    tile: colors.primary,
  },
];

export default function Onboarding(): React.ReactElement {
  const router = useRouter();
  const { width } = useWindowDimensions();
  const ref = useRef<ScrollView>(null);
  const [i, setI] = useState(0);
  const last = i === SLIDES.length - 1;

  const onScroll = (e: NativeSyntheticEvent<NativeScrollEvent>): void => {
    setI(Math.round(e.nativeEvent.contentOffset.x / width));
  };

  const next = (): void => {
    if (last) router.push('/(auth)/register');
    else ref.current?.scrollTo({ x: width * (i + 1), animated: true });
  };

  return (
    <Screen>
      {/* Header */}
      <View
        style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingTop: 4 }}
      >
        <Logo size={30} />
        <Pressable onPress={() => router.push('/(auth)/login')} hitSlop={8} style={{ minHeight: 44, justifyContent: 'center', paddingHorizontal: 8 }}>
          <Text variant="bodyStrong" color={colors.muted}>
            Skip
          </Text>
        </Pressable>
      </View>

      {/* Paged slides */}
      <ScrollView
        ref={ref}
        horizontal
        pagingEnabled
        showsHorizontalScrollIndicator={false}
        onMomentumScrollEnd={onScroll}
        style={{ flex: 1 }}
      >
        {SLIDES.map((s) => (
          <View key={s.title} style={{ width, justifyContent: 'center', paddingHorizontal: 20 }}>
            <LinearGradient
              colors={s.art}
              start={{ x: 0.1, y: 0 }}
              end={{ x: 0.9, y: 1 }}
              style={{
                height: 300,
                borderRadius: 26,
                alignItems: 'center',
                justifyContent: 'center',
                borderWidth: 1,
                borderColor: s.ai ? 'rgba(98,70,224,0.22)' : 'rgba(30,110,82,0.2)',
              }}
            >
              <View
                style={{
                  width: 116,
                  height: 116,
                  borderRadius: 34,
                  alignItems: 'center',
                  justifyContent: 'center',
                  backgroundColor: s.tile,
                  shadowColor: s.tile,
                  shadowOffset: { width: 0, height: 18 },
                  shadowOpacity: 0.5,
                  shadowRadius: 24,
                  elevation: 8,
                }}
              >
                <Icon name={s.icon} size={54} color="#FFFFFF" strokeWidth={1.7} fill={s.ai} />
              </View>
            </LinearGradient>

            <View style={{ marginTop: 30 }}>
              {s.ai ? <AILabel>Powered by AI</AILabel> : null}
              <Text variant="h1" style={{ marginTop: s.ai ? 10 : 0 }}>
                {s.title}
              </Text>
              <Text variant="body" color={colors.muted} style={{ marginTop: 12, fontSize: 15.5, lineHeight: 24 }}>
                {s.body}
              </Text>
            </View>
          </View>
        ))}
      </ScrollView>

      {/* Dots + CTA */}
      <View style={{ paddingHorizontal: 20, paddingBottom: 34 }}>
        <View style={{ flexDirection: 'row', gap: 8, marginBottom: 22 }}>
          {SLIDES.map((s, k) => (
            <View
              key={s.title}
              style={{ height: 7, borderRadius: 999, width: k === i ? 26 : 7, backgroundColor: k === i ? colors.primary : colors.line }}
            />
          ))}
        </View>
        <Button title={last ? 'Get started' : 'Next'} iconRight="fwd" onPress={next} />
      </View>
    </Screen>
  );
}
