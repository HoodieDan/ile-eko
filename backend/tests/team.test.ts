...
async function landlordToken(): Promise<string> {
  const res = await request(app).post('/v1/auth/register').send({
    name: 'Owner', email: `own-${randomUUID()}@example.com`, password: 'password123', role: 'landlord',
  });
...
async function setup() {
  ...
  const prop = await request(app).post('/v1/properties').set(auth(token)).send({
    propertyTitle: 'P', address: 'A', area: 'Yaba', lga: 'Lagos Mainland', propertyType: 'mini-flat', rentAmount: 900000,
  });
  ...
  const lease = await request(app).post('/v1/leases').set(auth(token)).send({
    tenantId: tenant.body.id, propertyId: prop.body.id, startDate: '2027-01-01', endDate: '2028-01-01', billingAmount: 900000, schedule: 'annual',
  });
...
describe('M3 team: invitation → accept → caretaker access', () => {
  it('invites, accepts, and enforces per-property permissions', async () => {
    ...
    const t = await request(app).post('/v1/tenants').set(auth(careToken)).send({ fullName: 'X', phone: '+2348019999999' });
    ...
    const create = await request(app).post('/v1/properties').set(auth(careToken)).send({
      propertyTitle: 'Z', address: 'A', area: 'B', lga: 'C', propertyType: 'other',
    });
  ...
  it('revoking a caretaker invalidates their session', async () => {
    const { token, propertyId } = await setup();
    const invite = await request(app)
      .post('/v1/team/invite')
      .set(auth(token))
      .set('Idempotency-Key', randomUUID())
      .send({ name: 'C', email: `c-${randomUUID()}@example.com`, grants: [{ propertyId, permissions: { canLogPayments: true } }] });
    const accept = await request(app).post('/v1/team/accept').send({ inviteToken: invite.body.token, name: 'C', password: 'password123' });
    const careToken = accept.body.token as string;
    ...
    const revoke = await request(app)
      ...
      .set(auth(token))
      .send({ propertyId, status: 'revoked' });
  ...
  it('rejects an invite for a property the landlord does not own', async () => {
    ...
    const invite = await request(app)
      ...
      .set(auth(b))
      .set('Idempotency-Key', randomUUID())
      .send({ name: 'C', email: `c-${randomUUID()}@example.com`, grants: [{ propertyId: a.propertyId, permissions: {} }] });
