...
describe('M2 ledger: standalone property lifecycle', () => {
  ...
  it('cannot create a second active lease on the same target', async () => {
    const prop = await request(app).post('/v1/properties').set(auth(token)).send({
      propertyTitle: 'P', address: 'A', area: 'Lekki', lga: 'Eti-Osa', propertyType: 'mini-flat', rentAmount: 1000000,
    });
    const t1 = await request(app).post('/v1/tenants').set(auth(token)).send({ fullName: 'T1', phone: '+2348000000001' });
    const t2 = await request(app).post('/v1/tenants').set(auth(token)).send({ fullName: 'T2', phone: '+2348000000002' });
    const base = { propertyId: prop.body.id, startDate: '2026-01-01', endDate: '2027-01-01', billingAmount: 1000000, schedule: 'annual' };
    const l1 = await request(app).post('/v1/leases').set(auth(token)).send({ ...base, tenantId: t1.body.id });
    expect(l1.status).toBe(201);
    const l2 = await request(app).post('/v1/leases').set(auth(token)).send({ ...base, tenantId: t2.body.id });
...
describe('M2 ledger: payments, allocation, idempotency, reversal', () => {
  ...
  beforeEach(async () => {
    token = await landlordToken();
    const prop = await request(app).post('/v1/properties').set(auth(token)).send({
      propertyTitle: 'P', address: 'A', area: 'Ikeja', lga: 'Ikeja', propertyType: 'two-bedroom', rentAmount: 1200000,
    });
    const tenant = await request(app).post('/v1/tenants').set(auth(token)).send({ fullName: 'Tayo', phone: '+2348011111111' });
    ...
    const lease = await request(app).post('/v1/leases').set(auth(token)).send({
      tenantId, propertyId: prop.body.id, startDate: '2027-01-01', endDate: '2028-01-01', billingAmount: 1200000, schedule: 'annual',
    });
    leaseId = lease.body.id;
  });

  it('requires an Idempotency-Key and records payment once on retry', async () => {
    const noKey = await request(app).post('/v1/payments').set(auth(token)).send({ leaseId, amount: 600000 });
    expect(noKey.status).toBe(400);

    const key = randomUUID();
    const first = await request(app).post('/v1/payments').set(auth(token)).set('Idempotency-Key', key).send({ leaseId, amount: 600000 });
    expect(first.status).toBe(201);
    const retry = await request(app).post('/v1/payments').set(auth(token)).set('Idempotency-Key', key).send({ leaseId, amount: 600000 });
    expect(retry.status).toBe(201);
    expect(retry.body.id).toBe(first.body.id); // same payment, not duplicated

    // A different body with the same key → 409
    const conflict = await request(app).post('/v1/payments').set(auth(token)).set('Idempotency-Key', key).send({ leaseId, amount: 999999 });
    expect(conflict.status).toBe(409);
  });

  it('partial then full payment moves obligation settlement partial → paid; tenant status updates', async () => {
    await request(app).post('/v1/payments').set(auth(token)).set('Idempotency-Key', randomUUID()).send({ leaseId, amount: 600000 });
  ...
  it('reversal offsets the payment (immutable ledger) and cannot double-reverse', async () => {
    const pay = await request(app).post('/v1/payments').set(auth(token)).set('Idempotency-Key', randomUUID()).send({ leaseId, amount: 1200000 });
    let t = await request(app).get(`/v1/tenants/${tenantId}`).set(auth(token));
    expect(t.body.status).toBe('up-to-date');

    const rev = await request(app).post(`/v1/payments/${pay.body.id}/reverse`).set(auth(token)).set('Idempotency-Key', randomUUID());
    ...
    const rev2 = await request(app).post(`/v1/payments/${pay.body.id}/reverse`).set(auth(token)).set('Idempotency-Key', randomUUID());
...
describe('M2: lease end, dashboard, archive guard', () => {
  it('ending a lease vacates the target and dashboard reflects occupancy', async () => {
    ...
    const prop = await request(app).post('/v1/properties').set(auth(token)).send({
      propertyTitle: 'P', address: 'A', area: 'Surulere', lga: 'Surulere', propertyType: 'mini-flat', rentAmount: 800000,
    });
    const tenant = await request(app).post('/v1/tenants').set(auth(token)).send({ fullName: 'Zed', phone: '+2348022222222' });
    const lease = await request(app).post('/v1/leases').set(auth(token)).send({
      tenantId: tenant.body.id, propertyId: prop.body.id, startDate: '2026-01-01', endDate: '2027-01-01', billingAmount: 800000, schedule: 'annual',
    });
